...
from rules import rule1_metadata, rule2_histogram, rule3_template, rule4_edges
...
class SimpleDetective:
    ...
    def find_best_match(self, input_image_path):
        ...
        for target_name, target_info in self.targets.items():
            ...
            # Apply Rule 4: Edges
            score4, fired4, evidence4 = rule4_edges(target_info, input_image_path)
        ...
        print(f"Rule 4 (Edges):     {'FIRED' if r4[0] else 'NO MATCH'} - {r4[1]} -> {r4[2]}/40 points")

        # Decide match vs rejected
        if best['score'] >= 60:  
            print(f"Final Score: {best['score']}/100 -> MATCH to {best['target']}")
            return {'best_match': best['target'], 'confidence': best['score']}
        else:
            print(f"Final Score: {best['score']}/100 -> REJECTED")
